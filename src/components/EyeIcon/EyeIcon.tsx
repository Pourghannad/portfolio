...
export default function EyeIcon(props: IEyeIcon) {
  ...
  const onMouseMove: MouseEventHandler = throttle((event: MouseEvent) => {
    ...
    if (element) {
      ...
      const elX = rect.left - event.clientX ;
      const elY = rect.top - event.clientY;
      setOffset({
        x: elX > 16 ? 16 : elX < -14 ? -14 : elX,
        y: elY > 18 ? 18 : elY < -14 ? -14 : elY,
      });
    }
  }, 20);
  ...
  return (
    ...
    >
      ...
      <path
        ...
        style={{
          transform: `scale(0.75) translate3D(${offset.x}px, ${offset.y}px, 0px)`,
        }}
        d="M58.16,47.37c-2.9,0-5.24-2.35-5.24-5.24,0-1.86,.97-3.49,2.43-4.42-1.64-.71-3.44-1.11-5.34-1.11-7.4,0-13.41,6-13.41,13.41s6,13.41,13.41,13.41,13.41-6,13.41-13.41c0-1.73-.33-3.38-.93-4.9-.95,1.37-2.53,2.27-4.32,2.27Z"
      />
    </svg>
  );
}
